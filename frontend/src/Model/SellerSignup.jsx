    ...
    async function handleSubmit() 
{
    ...
    try {
        ...
        //localStorage.setItem("username",data.useranme)
        setTimeout(()=>
        {
  ...
  return (
    <div className=' text-[#03045e] font-bold'>
        <div className='content-center w-[300px] ml-6 pt-6'>
            <h1 className='text-center text-4xl font-extrabold '><i className='rounded-md shadow-current shadow-md px-1 cursor-none'>Hotel Owner</i></h1>
        </div>
        ...
        <p className='text-gray-400 text-center cursor-none'>Enter your credentials to signup</p>
        </div>  
        ...
        <Input type="text" placeholder="FGHSJKD4" name="Id Proof" id="idproof" onChange={(e)=>handleChange("idProof",e)}  />
        <Input type="password" placeholder="****" name="Password" id="password" onChange={(e)=>handleChange("password",e)}  /> <br />
        ...
        <p className='text-gray-500 text-center'>Already have a account ? <a  className='underline cursor-pointer hover:text-[#03045e] ' onClick={()=>{authType("signin")}} >Signin</a></p>
        <Toaster />
        </div>
        </div>
        </div>
